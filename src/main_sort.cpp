...
int main(int argc, char **argv) {
	...
	*/
	...
	/*
	uint64_t post_split_checksum = distributed_checksum(rebalanced_data, rebalanced_data_size, confirm_mask, confirm_modulus, MPI_COMM_WORLD);
	if (rank==0){
		printf("Post findSplitters():\n");
		printf("checksum:%lu\n",post_split_checksum);
	}
	MPI_Barrier(MPI_COMM_WORLD);
	*/
	...
	/*
	uint64_t post_move_checksum = distributed_checksum(partitioned_data, partitioned_data_size, confirm_mask, confirm_modulus, MPI_COMM_WORLD);
	if (rank==0){
		printf("Post moveData():\n");
		printf("checksum:%lu\n",post_move_checksum);
	}
	MPI_Barrier(MPI_COMM_WORLD);
	*/
